...
async function generateContentWithRetry(params: any, maxRetries = 3, initialDelay = 800) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isTransient && attempt < maxRetries) {
        console.warn(`[Gemini API] Attempt ${attempt} failed with transient error (503/429). Retrying in ${delay}ms... Details:`, errorMessage);
  ...
  throw new Error("Gemini API call failed after multiple attempts/retries.");
}
...
// API: Get Fallback Tasks
app.get("/api/tasks/list", (req, res) => {
  res.json({ status: "success", data: initialTasks });
});

// API: Validate SCAMPER response with Gemini
...
`;
    ...
    const localTask = initialTasks.find(t => t.id === taskId);
...
`;
    ...
    console.error("Gemini Hint Endpoint Error:", error);
    const localTask = initialTasks.find(t => t.id === taskId);
...
`;
    ...
    console.error("Gemini Generate Task Error:", error);
    // Fallback to spawning a random offline task cloned with a new dynamic ID
    const matches = initialTasks.filter(t => t.topicId === topicId);
    const chosen = matches.length > 0 ? matches[Math.floor(Math.random() * matches.length)] : initialTasks[Math.floor(Math.random() * initialTasks.length)];
...
app.post("/api/ai/generate-task-by-slot", async (req, res) => {
  const { topicId, typeCode, index, topicName } = req.body;
...
`;
    ...
    console.error("Gemini Generate Slot Task Error:", error);
